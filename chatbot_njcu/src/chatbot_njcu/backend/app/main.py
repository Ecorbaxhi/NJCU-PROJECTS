...
def _find_csv_path() -> Path:
    ...
    for up in range(1, 8):  # look up to 7 directories up
        candidate = here.parents[up-1] / "data" / "qa.csv"
        if candidate.exists():
            return candidate
    # common fallback for repo layout (app -> backend -> chatbot_njcu -> src -> project -> data)
    return (here.parent.parent.parent.parent / "data" / "qa.csv")

DATA_CSV = _find_csv_path()

# define feedback file AFTER DATA_CSV exists
DATA_DIR = DATA_CSV.parent
FEEDBACK_CSV = DATA_DIR / "feedback.csv"

def _read_version_from_csv(path: Path) -> str:
    """
    If your first data row is like:
      question='VERSION OF CHATBOT?' | answer='OCT. 13 2025'
    return 'OCT. 13 2025', else '(unknown)'.
    """
    try:
...
def query(req: QueryRequest):
    ...
    # 2) fall back to normal semantic retrieval
